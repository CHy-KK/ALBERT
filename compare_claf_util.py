...
class SelfProcessor(DataProcessor):
	def get_train_examples(self, data_dir):
		file_path = os.path.join(data_dir, 'train_5neg.csv')
		...
		for _ , row in df_train.iterrows():
			index += 1
			guid = 'train-%d' % index
			text_a = self.process_text(str(row[5])).lower()
			abstract = str(row[3])
			title = str(row[4])
			if abstract != 'NO_CONTENT' and abstract != 'no_content' and abstract != 'nan':
				content = title + abstract
			else:
				content = title
			text_b = self.process_text(content).lower()
			label = str(row[2])
			examples.append(InputExample(guid=guid, text_a=text_a,
										text_b=text_b, label=label))
		return examples
		'''
		file_path_train = os.path.join(data_dir, 'train.csv')
		file_path_candidate = os.path.join(data_dir,'candidate_paper_for_wsdm2020.csv')
		df = pd.read_csv(file_path_train)
		df_candidate = pd.read_csv(file_path_candidate)
		...
		cnt = 0
			...
			df_train :
			row[0] : decription id
			row[1] : paper id(ground truth)
			row[2] : description text
		
		for index, row in (df_train.iterrows()):
			guid = 'train-ture-%d' % index
			text_a = tokenization.convert_to_unicode(str(row[2]))  #description text
			paper = df_candidate[df_candidate['paper_id'] == row[1]].index.tolist()
			if not paper:
				continue
			abstract = str(df_candidate.iloc[paper[0]].values[0])
			#print(abstract)
			if abstract != 'NO_CONTENT' and abstract != 'no_content' and abstract != np.nan:
				#print(df_candidate.loc[paper[0],'title'])
				content = str(df_candidate.loc[paper[0]]['title']) + abstract
			else:
				content = str(df_candidate.loc[paper[0]]['title'])
			text_b = tokenization.convert_to_unicode(str(content))  #paper's abstract
			label = "1"  #label
			examples.append(InputExample(guid=guid, text_a=text_a,
											text_b=text_b, label=label))
			#randomly choose false examples from df_train
			for _ in range(2):
				guid = 'train-false-%d' % cnt
				cnt += 1
				col = random.randint(3,df_train.shape[1] - 1)
				paper_id = row[col]
				paper = df_candidate[df_candidate['paper_id'] == paper_id].index.tolist()
				if not paper:
					continue
				abstract = str(df_candidate.iloc[paper[0]].values[0])
				if abstract != 'NO_CONTENT' and abstract != 'no_content' and abstract != np.nan:
					...
					content = str(df_candidate.loc[paper[0]]['title'])
				text_c = tokenization.convert_to_unicode(str(content))
				label = "0"
				examples.append(InputExample(guid=guid, text_a=text_a,
											text_b=text_c, label=label))
	...
	def get_dev_examples(self, data_dir):
		...
		for index, row in self.df_dev.iterrows():
			guid = 'dev-%d' % index
			text_a = tokenization.convert_to_unicode(str(row[2]))
			paper = df_candidate[df_candidate['paper_id'] == row[1]].index.tolist()
			if not paper:
				continue
			abstract = str(df_candidate.iloc[paper[0]].values[0])
			if abstract != 'NO_CONTENT' and abstract != 'no_content' and abstract != np.nan:
				...
				content = str(df_candidate.loc[paper[0]]['title'])
			text_b = tokenization.convert_to_unicode(str(content))
			label = "1"
			examples.append(InputExample(guid=guid, text_a=text_a,
											text_b=text_b, label=label))
		return examples
	...
	#prediction
	def get_test_examples(self, data_dir):
		file_path = data_dir #os.path.join(data_dir, 'Lite_validation.csv')
		df_test = pd.read_csv(file_path)
		index_des = -1
		examples = []
		for _ , row in df_test.iterrows():
			index_des += 1
			guid = 'test-%d' % index_des
			text_a = self.process_text(str(row[4])).lower()
			abstract = str(row[2])
			title = str(row[3])
			if abstract != 'NO_CONTENT' and abstract != 'no_content' and abstract != 'nan':
				content = title + abstract
			...
			'''for i in range(2,df_test.shape[1]):
				if row[i] == None:
					break
				guid = 'test-%d-%d' % index_des,index_cand
				index_cand += 1
				paper = df_candidate[df_candidate['paper_id'] == row[i]].index.tolist()
				if not paper:
					continue
				abstract = str(df_candidate.iloc[paper[0]].values[0])
				if abstract != 'NO_CONTENT' and abstract != 'no_content' and abstract != np.nan:
					content = str(df_candidate.iloc[paper[0]]['title']) + abstract
				else:
					content = str(df_candidate.iloc[paper[0]]['title'])
					text_b = tokenization.convert_to_unicode(str(content))
					label = "0"
					examples.append(InputExample(guid=guid, text_a=text_a,
												text_b=text_b, label=label))'''
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, task_name, tokens_stride=64):
	...
	max_tokens_length = ((max_seq_length) - 3) / 2
	...
	if tokens_b:
		...
		while start_offset < len(tokens_a):
			...
			if length > max_tokens_length:
				length = max_tokens_length
			tokens_a_list.append(tokens_a[start_offset:start_offset + length])
			if start_offset + length == len(tokens_a)
				break
			start_offset += tokens_stride
		
         	start_offset = 0
		while start_offset < len(tokens_b):
			length = len(tokens_b) - start_offset
			if length > max_tokens_length:
				length = max_tokens_length
			tokens_b_list.append(tokens_b[start_offset:start_offset + length])
			if start_offset + length == len(tokens_b)
				break
			start_offset += tokens_stride
...
def file_based_convert_examples_to_features(
		examples, label_list, max_seq_length, tokenizer, output_file, task_name, tokens_stride):
	...
	for (ex_index, example) in enumerate(examples):
		...
		feature_list = convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, task_name, tokens_stride)

		batch_index.extend([ex_index]*len(feature_list)) 
	...
	def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			...
			train_op = optimization.create_optimizer(
					...
					use_tpu, optimizer)
			logging_hook = tf.train.LoggingTensorHook({"loss": total_loss}, every_n_iter=2000)
